...
export default {
  valid: () =>
    withLocalTmpDir(async () => {
      ...
      const output = await execaCommand('base prepublishOnly', { all: true })
      expect(
        globby('**', { cwd: 'dist', dot: true, filesOnly: false })
          |> await
          |> sortBy(identity),
      ).toEqual(['foo/test.scss', 'index.scss', 'test.txt'])
      expect(await fs.readFile(P.resolve('dist', 'foo', 'test.scss'), 'utf8'))
        .toEqual(endent`
          $color: blue
          body
            color: $color
        `)
      expect(await fs.readFile(P.resolve('dist', 'index.scss'), 'utf8'))
        .toEqual(endent`
          $color: red;
          body {
            background: $color;
